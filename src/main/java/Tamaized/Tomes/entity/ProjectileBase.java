...
public abstract class ProjectileBase extends EntityArrow implements IProjectile, IEntityAdditionalSpawnData {
	...
	protected void onHit(RayTraceResult raytraceResultIn) {
		...
		if (entity != null) {
			if (entity == shootingEntity || !canHitEntity(entity))
				return;
	...
	public void readEntityFromNBT(NBTTagCompound compound) {
		...
		if (compound.hasKey("pickup", 99)) {
			pickupStatus = PickupStatus.getByOrdinal(compound.getByte("pickup"));
		} else if (compound.hasKey("player", 99)) {
			pickupStatus = compound.getBoolean("player") ? PickupStatus.ALLOWED : PickupStatus.DISALLOWED;
		}
	}
